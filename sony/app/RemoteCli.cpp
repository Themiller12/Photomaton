...
int main(int argc, char** argv)
{
    ...
    int forcedIndex = 1; // 1-based
    int waitAfterMs = 2500;
    for(int i=1;i<argc;i++){
        std::string a = argv[i];
        if(a == "--auto-capture" || a == "--auto") autoMode = true;
        else if(a == "--capture-once") captureOnce = true;
        else if(a.rfind("--index=",0)==0){ forcedIndex = std::atoi(a.substr(8).c_str()); if(forcedIndex<=0) forcedIndex=1; }
        else if(a.rfind("--wait=",0)==0){ waitAfterMs = std::atoi(a.substr(7).c_str()); if(waitAfterMs<500) waitAfterMs=500; }
    }
    ...
    cli::text connectNo;
    if(autoMode) {
        connectNo = std::to_wstring((long long)forcedIndex).c_str();
    } else {
    ...
    if(autoMode){
        // Auto mode: connect and optionally capture once then exit.
        if(!camera->connect(SDK::CrSdkControlMode_Remote, SDK::CrReconnecting_OFF)){
            cli::tout << "Auto connect failed.\n";
            SDK::Release();
            return 2;
        }
        cli::tout << "Auto connected.\n";
        if(captureOnce){
            cli::tout << "Capturing...\n";
            camera->capture_image();
            // Portable sleep instead of platform-specific Sleep/usleep
            std::this_thread::sleep_for(std::chrono::milliseconds(waitAfterMs));
            camera->disconnect();
            cli::tout << "Done.\n";
        }
        SDK::Release();
